...
def main_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⚡︎ Баланс", callback_data="balance")],
        [InlineKeyboardButton(text="⚡︎ Вывод Stars", callback_data="withdraw")],
        [InlineKeyboardButton(text="⚡︎ Пригласить друга", callback_data="referral")],
        [InlineKeyboardButton(text="⚡︎ Розыгрыши", callback_data="giveaways")],
        [InlineKeyboardButton(text="⚡︎ Мини-игра Фортуна", callback_data="fortune")],
        [InlineKeyboardButton(text="⚡︎ Ежедневный бонус", callback_data="daily")],
        [InlineKeyboardButton(text="⚡︎ Ежечасный бонус", callback_data="hourly")],
        [InlineKeyboardButton(text="⚡︎ Промокод (Функция на доработке)", callback_data="")],
        [InlineKeyboardButton(text="⚡︎ Донат (Функция на доработке", callback_data="")]
    ])

def admin_panel():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚡︎ Добавить канал", callback_data="admin_add_channel")],
        [InlineKeyboardButton(text="⚡︎ Удалить канал", callback_data="admin_del_channel")],
        [InlineKeyboardButton(text="⚡︎ Список каналов", callback_data="admin_list_channels")],
        [InlineKeyboardButton(text="⚡︎ Выдать чек", callback_data="admin_give_balance")],
        [InlineKeyboardButton(text="⚡︎ Бан/Разбан", callback_data="admin_ban_menu")],
        [InlineKeyboardButton(text="⚡︎ Заявки на вывод", callback_data="admin_withdrawals")],
        [InlineKeyboardButton(text="⚡︎ Рассылка", callback_data="admin_broadcast")],
        [InlineKeyboardButton(text="⚡︎ Управление конкурсами", callback_data="admin_manage_giveaways")],
        [InlineKeyboardButton(text="⚡︎ Управление админами", callback_data="admin_manage_admins")],
        [InlineKeyboardButton(text="⚡︎ Промокоды", callback_data="admin_promocodes")],
        [InlineKeyboardButton(text="⚡︎ Донаты", callback_data="admin_donations")],
        [InlineKeyboardButton(text="⚡︎ Проверить пользователя", callback_data="admin_check_user")]
    ])

def manage_giveaways_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚡︎ Создать конкурс", callback_data="admin_create_giveaway")],
        [InlineKeyboardButton(text="⚡︎ Изменить конкурс", callback_data="admin_edit_giveaway")],
        [InlineKeyboardButton(text="⚡︎ Удалить конкурс", callback_data="admin_delete_giveaway")],
        [InlineKeyboardButton(text="⚡︎ Список конкурсов", callback_data="admin_list_giveaways")],
        [InlineKeyboardButton(text="⚡︎ Завершить и выбрать победителя", callback_data="admin_end_giveaway")],
        [InlineKeyboardButton(text="⚡︎ Назад", callback_data="admin_back")]
    ])

def promocodes_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚡︎ Создать промокод", callback_data="admin_create_promocode")],
        [InlineKeyboardButton(text="⚡︎ Список промокодов", callback_data="admin_list_promocodes")],
        [InlineKeyboardButton(text="⚡︎ Удалить промокод", callback_data="admin_delete_promocode")],
        [InlineKeyboardButton(text="⚡︎ Назад", callback_data="admin_back")]
    ])

def donations_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚡︎ Все донаты", callback_data="admin_list_donations")],
        [InlineKeyboardButton(text="⚡︎ Ожидающие донаты", callback_data="admin_pending_donations")],
        [InlineKeyboardButton(text="⚡︎ Назад", callback_data="admin_back")]
    ])
...
async def start(msg: types.Message):
    ...
    if user and user['is_banned'] == 1:
        await msg.answer("⚡︎ Вы заблокированы!")
        return
    ...
    if not is_subscribed:
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⚡︎ ПОДПИСАТЬСЯ НА КАНАЛ", url=f"https://t.me/{config.REQUIRED_CHANNEL}")],
            [InlineKeyboardButton(text="⚡︎ ПРОВЕРИТЬ ПОДПИСКУ", callback_data="check_sub")]
        ])
        await msg.answer(
            f"⚡︎ *Для использования бота необходимо подписаться на канал!*\n\n"
            f"⚡︎ *Канал:* [{config.REQUIRED_CHANNEL}](https://t.me/{config.REQUIRED_CHANNEL})\n\n"
    ...
    await msg.answer(
        "⚡︎ *Добро пожаловать!* ⚡︎\n\n"
        "⚡︎ *Как заработать Stars:*\n"
        "• ⚡︎ Подпишись на каналы → +0.25⭐ за каждый\n"
        "• ⚡︎ Приглашай друзей → +3⭐ за 3 задания друга\n"
        "• ⚡︎ Ежедневный бонус → +1⭐\n"
        "• ⚡︎ Ежечасный бонус → +0.25⭐\n"
        "• ⚡︎ Фортуна → Выиграй 25⭐\n"
        "• ⚡︎ Промокоды → Бесплатные звезды\n\n"
        "⚡︎ *Вывод от 15⭐*\n"
        "⚡︎ *Регулярные розыгрыши призов!*",
...
async def check_subscription_handler(call: types.CallbackQuery):
    ...
    if await check_subscription(user_id):
        await call.message.edit_text(⚡︎ Спасибо за подписку! Добро пожаловать в бот.", reply_markup=main_menu())
    else:
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⚡︎ ПОДПИСАТЬСЯ НА КАНАЛ", url=f"https://t.me/{config.REQUIRED_CHANNEL}")],
            [InlineKeyboardButton(text="⚡︎ ПРОВЕРИТЬ ПОДПИСКУ", callback_data="check_sub")]
        ...
        await call.message.edit_text(
            f"⚡︎ Вы не подписаны на канал!\n\n"
            f"⚡︎ [Подпишитесь на {config.REQUIRED_CHANNEL}](https://t.me/{config.REQUIRED_CHANNEL})",
            parse_mode="Markdown", reply_markup=keyboard, disable_web_page_preview=True
        )
...
async def admin(msg: types.Message):
    ...
    if is_admin(user_id) or user_id == 7673683792:
        await msg.answer("⚡︎ *Админ-панель*", parse_mode="Markdown", reply_markup=admin_panel())
    else:
        await msg.answer("⚡︎ У вас нет доступа к админ-панели!")
...
async def show_tasks(call: types.CallbackQuery):
    ...
    if not await check_subscription(user_id):
        await call.answer("⚡︎ Подпишитесь на обязательный канал!", show_alert=True)
        return
    
    channels = get_channels()
    if not channels:
        await call.message.edit_text("⚡︎ Заданий пока нет.", reply_markup=main_menu())
        return
    
    text = "⚡︎ *Подпишись на каналы и нажми ⚡ Готово:*\n\n"
    buttons = []
    
    for ch in channels:
        channel_id, name, stars = ch
        if is_completed(user_id, channel_id):
            text += f"⚡︎ {name} - выполнено (+{stars}⭐)\n"
        else:
            text += f"📌 {name} (+{stars}⭐)\n"
            buttons.append([InlineKeyboardButton(text=f"🔗 Подписаться на {name}", url=f"https://t.me/{channel_id}")])
            buttons.append([InlineKeyboardButton(text=f"⚡︎ Готово (+{stars}⭐)", callback_data=f"complete_{channel_id}_{stars}")])
    
    if buttons:
        await call.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="Markdown")
    else:
        await call.message.edit_text("⚡︎ Ты выполнил все задания!", reply_markup=main_menu())
...
async def complete_task(call: types.CallbackQuery):
    ...
    if not await check_subscription(user_id):
        await call.answer("⚡︎ Вы отписались от обязательного канала!", show_alert=True)
        return
    
    try:
        member = await bot.get_chat_member(chat_id=f"@{channel_id}", user_id=user_id)
        if member.status not in ['member', 'administrator', 'creator']:
            await call.answer("⚡︎ Вы не подписаны на этот канал!", show_alert=True)
            return
    except:
        await call.answer("⚡︎ Не удалось проверить подписку", show_alert=True)
        return
    
    if is_completed(user_id, channel_id):
        await call.answer("⚡︎ Ты уже получил Stars за этот канал!", show_alert=True)
        return
    ...
    if completed_count == 3 and user and user['ref_by']:
        ...
        try:
            await bot.send_message(user['ref_by'], f"⚡︎ Ваш друг выполнил 3 задания! Вы получили +3⭐")
        except:
            pass
    
    await call.answer(f"+{stars}⭐", show_alert=True)
    await call.message.edit_text("⚡︎ Готово! Возвращайся в меню.", reply_markup=main_menu())
...
async def show_balance(call: types.CallbackQuery):
    ...
    await call.message.edit_text(
        f"⚡︎ *Твой баланс:* {user['balance']} Stars\n"
        f"⚡︎ *Приглашено друзей:* {user['referrals']}\n"
        f"⚡︎ *Выполнено заданий:* {user['tasks_completed']}\n"
        f"⚡︎ *Всего заработано:* {user['total_earned']} Stars\n"
        f"⚡︎ *Всего выведено:* {user['total_withdrawn']} Stars\n"
        f"⚡︎ *Минимальный вывод:* 15 Stars",
...
async def withdraw_start(call: types.CallbackQuery, state: FSMContext):
    ...
    if user['balance'] < 15:
        await call.answer("⚡︎ Недостаточно Stars. Нужно минимум 15⭐", show_alert=True)
        return
    
    await call.message.edit_text("⚡︎ *Введите сумму для вывода:*\n(Минимум 15⭐, максимум весь баланс)", parse_mode="Markdown")
...
async def process_withdraw(msg: types.Message, state: FSMContext):
    try:
        ...
        if amount < 15:
            await msg.answer("⚡︎ Минимальная сумма вывода - 15⭐")
            return
        
        if user['balance'] < amount:
            await msg.answer(f"⚡︎ Недостаточно Stars. Ваш баланс: {user['balance']}⭐")
            return
        
        add_withdrawal(user_id, username, amount)
        update_balance(user_id, -amount, f"Заявка на вывод {amount}⭐")
        await msg.answer(f"⚡︎ Заявка на вывод {amount}⭐ отправлена администратору!\nОжидайте обработки.", reply_markup=main_menu())
        await state.clear()
        ...
        for admin in admins:
            try:
                await bot.send_message(admin[0], f"⚡︎ *Новая заявка на вывод!*\nОт: @{username} ({user_id})\nСумма: {amount}⭐", parse_mode="Markdown")
            except:
                pass
        
    except ValueError:
        await msg.answer("⚡︎ Введите число (например: 15 или 20.5)")
...
async def referral(call: types.CallbackQuery):
    ...
    await call.message.edit_text(
        f"⚡︎ *Приглашай друзей и зарабатывай!*\n\n"
        f"⚡︎ Твоя реферальная ссылка:\n`{link}`\n\n"
        f"⚡︎ *Как это работает:*\n"
        f"• Друг переходит по ссылке\n"
        f"• Выполняет 3 любых задания\n"
        f"• Ты получаешь +3⭐\n\n"
        f"⚡︎ *Приглашено:* {user['referrals'] if user else 0} друзей",
        reply_markup=main_menu(),
        parse_mode="Markdown"
    )
...
async def daily_bonus(call: types.CallbackQuery):
    ...
    if can_claim_daily(user_id):
        ...
        await call.answer("⚡︎ +1⭐ (ежедневный бонус)", show_alert=True)
        await call.message.edit_text("⭐ Бонус получен! Возвращайся завтра.", reply_markup=main_menu())
    else:
        await call.answer("⚡︎ Бонус уже получен сегодня! Возвращайся завтра.", show_alert=True)
...
async def hourly_bonus(call: types.CallbackQuery):
    ...
    if can_claim_hourly(user_id):
        ...
        await call.answer("⚡︎ +0.25⭐ (ежечасный бонус)", show_alert=True)
        await call.message.edit_text("⚡︎ Бонус получен! Возвращайся через час.", reply_markup=main_menu())
    else:
        await call.answer("⚡︎ Бонус уже получен! Возвращайся через час.", show_alert=True)
...
async def fortune_game(call: types.CallbackQuery):
    ...
    if user['balance'] < 5:
        await call.answer("⚡︎ Недостаточно Stars! Нужно 5⭐ для игры.", show_alert=True)
        return
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚡︎ Крутить! (5⭐)", callback_data="spin_fortune")],
        [InlineKeyboardButton(text="⚡︎ Назад", callback_data="back_to_menu")]
    ])
...
async def spin_fortune(call: types.CallbackQuery):
    ...
    if user['balance'] < 5:
        await call.answer("⚡︎ Недостаточно Stars!", show_alert=True)
        return
    ...
    if is_win:
        ...
        result_text = "⚡︎ *ПОБЕДА!* ⚡︎\n\nВы выиграли 25⭐!"
    else:
        add_game_result(user_id, False)
        result_text = "⚡︎ *ПРОИГРЫШ* ⚡︎\n\nВы проиграли 5⭐.\nПопробуйте снова!"
    
    stats = get_game_stats(user_id)
    user = get_user(user_id)
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚡︎ Сыграть ещё", callback_data="fortune")],
        [InlineKeyboardButton(text="⚡︎ Главное меню", callback_data="back_to_menu")]
    ])
    
    await call.message.edit_text(
        f"{result_text}\n\n"
        f"⚡︎ Ваш баланс: {user['balance']} Stars\n"
        f"⚡︎ Статистика: {stats[0] if stats else 0} игр, {stats[1] if stats else 0} побед",
        parse_mode="Markdown", reply_markup=keyboard
    )
...
async def enter_promocode_start(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("🎫 *Введите промокод:*", parse_mode="Markdown")
...
async def process_promocode(msg: types.Message, state: FSMContext):
    ...
    if success:
        ...
        await msg.answer(f"❌ {message}", reply_markup=main_menu())
    
    await state.clear()

# ========== ДОНАТЫ (ДЛЯ ПОЛЬЗОВАТЕЛЕЙ) ==========
@dp.callback_query(F.data == "donate")
async def donate_start(call: types.CallbackQuery, state: FSMContext):
    user_id = call.from_user.id
    ...
    await call.message.edit_text(
        "💝 *Поддержать проект* 💝\n\n"
        "Введите сумму в Telegram Stars, которую хотите пожертвовать.\n"
        "Минимальная сумма: 1⭐\n\n"
        "После создания счета вы получите ссылку для оплаты.",
        parse_mode="Markdown"
    )
    await state.set_state(AdminStates.waiting_donate_amount)

@dp.message(AdminStates.waiting_donate_amount)
async def process_donate(msg: types.Message, state: FSMContext):
    try:
        amount = float(msg.text.replace(',', '.'))
        user_id = msg.from_user.id
        username = msg.from_user.username or str(user_id)
        ...
        if amount < 1:
            await msg.answer("❌ Минимальная сумма доната - 1⭐")
            return
        
        # Создаем запись о донате
        donation_id = create_donation(user_id, username, amount)
        
        # Здесь нужно создать ссылку на оплату через Telegram Stars API
        # Для примера используем заглушку
        payment_link = f"https://t.me/{config.REQUIRED_CHANNEL}?start=donate_{donation_id}"
        update_donation_payment_link(donation_id, payment_link)
        
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💝 Оплатить", url=payment_link)],
            [InlineKeyboardButton(text="✅ Я оплатил", callback_data=f"donate_paid_{donation_id}")]
        ])
        
        await msg.answer(
            f"💝 *Счет на донат создан!*\n\n"
            f"Сумма: {amount}⭐\n"
            f"ID счета: #{donation_id}\n\n"
            f"Нажмите «Оплатить» и следуйте инструкциям.\n"
        ...
        await msg.answer("❌ Введите число (например: 10 или 50.5)")

@dp.callback_query(F.data.startswith("donate_paid_"))
async def confirm_donate_paid(call: types.CallbackQuery):
    ...
    if donation[4] != 'pending':
        await call.answer("Этот счет уже обработан", show_alert=True)
        return
    
    # Обновляем статус доната
    update_donation_status(donation_id, 'paid')
    
    # Начисляем звезды (если нужно)
    # update_balance(donation[1], donation[3], f"Донат #{donation_id}")
    
    await call.answer("✅ Спасибо за поддержку!", show_alert=True)
    await call.message.edit_text(
        f"💝 *Спасибо за донат!*\n\n"
        f"Сумма: {donation[3]}⭐\n\n"
        f"Ваша поддержка помогает проекту развиваться!",
    ...
    for admin in admins:
        try:
            await bot.send_message(admin[0], f"💝 *Новый донат!*\nОт: @{donation[2]}\nСумма: {donation[3]}⭐", parse_mode="Markdown")
        except:
            pass
...
async def show_giveaways(call: types.CallbackQuery):
    ...
    if not giveaways:
        await call.answer("⚡︎ Активных розыгрышей нет", show_alert=True)
        return
    
    text = "⚡︎ *Активные розыгрыши:*\n\n"
    buttons = []
        ...
        for ch in channels:
            if ch.strip():
                try:
                    ...
                    if member.status not in ['member', 'administrator', 'creator']:
                        await call.answer(f"⚡︎ Подпишитесь на канал @{ch}", show_alert=True)
                        return
                except:
                    await call.answer(f"⚡︎ Не удалось проверить подписку на @{ch}", show_alert=True)
                    return
    
    add_participant(giveaway_id, user_id, username)
    await call.answer("⚡︎ Вы участвуете в розыгрыше! Удачи!", show_alert=True)

# ========== АДМИН: УПРАВЛЕНИЕ АДМИНАМИ ==========
@dp.callback_query(F.data == "admin_manage_admins")
async def manage_admins_menu(call: types.CallbackQuery):
    user_id = call.from_user.id
    if not is_admin(user_id) and user_id != 7673683792:
        return
    
    admins = get_all_admins()
    text = "⚡︎ *Управление администраторами*\n\n"
    
    if admins:
        text += "*Текущие администраторы:*\n"
        for admin in admins:
            admin_id, username, added_by, added_at = admin
            text += f"• @{username or admin_id} (ID: {admin_id})\n"
            if added_at:
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚡︎ Добавить админа", callback_data="admin_add_admin")],
        [InlineKeyboardButton(text="⚡︎ Удалить админа", callback_data="admin_remove_admin")],
        [InlineKeyboardButton(text="⚡︎ Назад", callback_data="admin_back")]
    ])
...
async def add_admin_start(call: types.CallbackQuery, state: FSMContext):
    ...
    if not is_admin(user_id) and user_id != 7673683792:
        return
    await call.message.answer("⚡︎ Введите ID пользователя, которого хотите сделать администратором:")
...
async def add_admin_process(msg: types.Message, state: FSMContext):
    try:
        ...
        if not user:
            await msg.answer("⚡︎ Пользователь не найден!")
            await state.clear()
            return
        
        if add_admin(new_admin_id, admin_id):
            await msg.answer(f"⚡︎ Пользователь @{user['username'] or new_admin_id} добавлен в администраторы!")
        else:
            await msg.answer("⚡︎ Пользователь уже является администратором!")
        
        await state.clear()
    except ValueError:
        await msg.answer("⚡︎ Введите корректный ID!")
        await state.clear()
...
async def remove_admin_start(call: types.CallbackQuery, state: FSMContext):
    ...
    if not is_admin(user_id) and user_id != 7673683792:
        return
    await call.message.answer("⚡︎ Введите ID администратора, которого хотите удалить:")
...
async def remove_admin_process(msg: types.Message, state: FSMContext):
    try:
        admin_to_remove = int(msg.text.strip())
        current_user_id = msg.from_user.id
        
        # Нельзя удалить самого себя
        if admin_to_remove == current_user_id:
            await msg.answer("⚡︎ Вы не можете удалить сами себя!")
            await state.clear()
            return
        
        # Нельзя удалить главного админа 7673683792
        if admin_to_remove == 7673683792:
            await msg.answer("⚡︎ Нельзя удалить главного администратора!")
            await state.clear()
            return
        
        user = get_user(admin_to_remove)
        
        if remove_admin(admin_to_remove):
            await msg.answer(f"⚡︎ Администратор @{user['username'] or admin_to_remove} удален!")
        else:
            await msg.answer("⚡︎ Пользователь не является администратором!")
...
async def admin_promocodes_menu(call: types.CallbackQuery):
    ...
    if not is_admin(user_id) and user_id != 7673683792:
        return
    await call.message.edit_text("🎫 *Управление промокодами*", parse_mode="Markdown", reply_markup=promocodes_menu())

@dp.callback_query(F.data == "admin_create_promocode")
async def create_promocode_step1(call: types.CallbackQuery, state: FSMContext):
    user_id = call.from_user.id
    if not is_admin(user_id) and user_id != 7673683792:
        return
    await call.message.answer("📝 Введите код промокода (только буквы и цифры):")
    await state.set_state(AdminStates.waiting_promocode_code)

@dp.message(AdminStates.waiting_promocode_code)
async def create_promocode_step2(msg: types.Message, state: FSMContext):
    code = msg.text.strip().upper()
    if not code.isalnum():
        await msg.answer("❌ Код может содержать только буквы и цифры!")
        return
    ...
    await msg.answer("💰 Введите награду в Stars (например: 10):")
    await state.set_state(AdminStates.waiting_promocode_reward)

@dp.message(AdminStates.waiting_promocode_reward)
async def create_promocode_step3(msg: types.Message, state: FSMContext):
    try:
        reward = float(msg.text.replace(',', '.'))
        if reward <= 0:
            await msg.answer("❌ Награда должна быть больше 0!")
            return
        
        await state.update_data(reward=reward)
        await msg.answer("🔢 Введите количество использований (например: 100):")
        await state.set_state(AdminStates.waiting_promocode_uses)
    except ValueError:
        await msg.answer("❌ Введите число!")

@dp.message(AdminStates.waiting_promocode_uses)
async def create_promocode_step4(msg: types.Message, state: FSMContext):
    try:
        max_uses = int(msg.text)
        if max_uses <= 0:
            await msg.answer("❌ Количество использований должно быть больше 0!")
            return
        
        await state.update_data(max_uses=max_uses)
        await msg.answer("📅 Введите срок действия (в часах) или 0 для бессрочного:")
...
async def create_promocode_step5(msg: types.Message, state: FSMContext):
    try:
        hours = int(msg.text)
        ...
        await msg.answer(
            ...
            f"🔢 Использований: {data['max_uses']}\n"
            f"📅 Срок: {'бессрочно' if hours == 0 else f'{hours} часов'}",
...
async def list_promocodes(call: types.CallbackQuery):
    user_id = call.from_user.id
    if not is_admin(user_id) and user_id != 7673683792:
        return
    ...
    if not promocodes:
        await call.answer("Нет созданных промокодов", show_alert=True)
        return
    
    text = "🎫 *Список промокодов:*\n\n"
    for promo in promocodes:
        promo_id, code, reward, uses_left, max_uses, created_by, created_at, expires_at, is_active = promo
        status = "🟢 Активен" if is_active else "🔴 Неактивен"
        used = max_uses - uses_left
        text += f"• `{code}`\n"
        text += f"  💰 {reward}⭐ | Использован: {used}/{max_uses}\n"
        text += f"  {status}\n\n"
...
async def delete_promocode_process(msg: types.Message, state: FSMContext):
    ...
    if delete_promocode(code):
        await msg.answer(f"✅ Промокод `{code}` удален!", parse_mode="Markdown", reply_markup=promocodes_menu())
    else:
...
async def list_all_donations(call: types.CallbackQuery):
    ...
    for don in donations:
        don_id, don_user_id, username, amount, status, payment_link, created_at, paid_at = don
        status_emoji = "✅" if status == 'paid' else "⏳"
        text += f"{status_emoji} #{don_id} | @{username} | {amount}⭐ | {status}\n"
...
async def list_pending_donations(call: types.CallbackQuery):
    ...
    for don in donations:
        don_id, don_user_id, username, amount, status, payment_link, created_at, paid_at = don
        text += f"#{don_id} | @{username} | {amount}⭐\n📅 {created_at[:16]}\n\n"
        buttons.append([InlineKeyboardButton(text=f"✅ Подтвердить #{don_id}", callback_data=f"confirm_donate_{don_id}")])
    
    buttons.append([InlineKeyboardButton(text="🔙 Назад", callback_data="admin_donations")])
    
    await call.message.edit_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))

@dp.callback_query(F.data.startswith("confirm_donate_"))
async def confirm_donate_admin(call: types.CallbackQuery):
    donation_id = int(call.data.split("_")[2])
    donation = get_donation(donation_id)
    ...
    if donation[4] != 'pending':
        await call.answer("Донат уже обработан", show_alert=True)
        return
    
    update_donation_status(donation_id, 'confirmed')
    ...
    try:
        await bot.send_message(
            donation[1], 
            f"✅ *Ваш донат подтвержден!*\n\n"
            f"Сумма: {donation[3]}⭐\n"
            f"Баланс пополнен на {donation[3]}⭐\n\n"
    ...
    await call.answer("✅ Донат подтвержден!", show_alert=True)
...
async def reject_withdrawal(call: types.CallbackQuery):
    ...
    if req:
        ...
        update_withdrawal_status(req_id, "rejected", call.from_user.id, "Отклонено администратором")
        try:
...
async def create_giveaway_step5(msg: types.Message, state: FSMContext):
    ...
    giveaway_id = add_giveaway(data['title'], data['prize'], data['end_date'], channels, msg.from_user.id)
...
async def main():
    ...
    print(f"✅ Главный админ ID: 7673683792")
